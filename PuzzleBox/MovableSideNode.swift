...
class MovableSideNode: SCNNode {
    ...
    init(width: Double, height: Double, isLeft: Bool) {
        super.init()
        let thickness = Wall.thickness
        let outsideHeight = isLeft ? height : height - thickness
        let yPosition = isLeft ? Float(thickness) / 2 : 0
        
        let outside = SCNBox(width: Wall.thickness, height: outsideHeight, length: width - 2 * thickness, chamferRadius: 0)
        outside.firstMaterial?.diffuse.contents = Wall.outsideColors[MovableSideNode.number]
        let outsideNode = SCNNode(geometry: outside)
        outsideNode.position = SCNVector3(x: -Float(thickness), y: 0, z: 0)
        outsideNode.physicsBody = SCNPhysicsBody.static()
        addChildNode(outsideNode)
        
        let middle = SCNBox(width: Wall.thickness, height: height - 3 * thickness, length: width - 4 * thickness, chamferRadius: 0)
        ...
        let inside = SCNBox(width: Wall.thickness, height: height - 5 * thickness, length: width - 2 * thickness, chamferRadius: 0)
        inside.firstMaterial?.diffuse.contents = Wall.insideColors[MovableSideNode.number]
        let insideNode = SCNNode(geometry: inside)
        insideNode.position = SCNVector3(x: Float(thickness), y: yPosition, z: 0)
